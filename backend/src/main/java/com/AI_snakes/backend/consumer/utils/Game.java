...
public class Game extends Thread {
    ...
    private boolean nextStep() { // wait for the operations of two players
        try {
            Thread.sleep(200);
    ...
    private void sendMove() { // send moves to the two clients
        lock.lock();
        try {
            JSONObject resp = new JSONObject();
            resp.put("event", "move");
    ...
    private boolean check_valid(List<Cell> cellsA, List<Cell> cellsB) {
        ...
        if (hasWall[cell.x][cell.y] == 1) return false;
